...
public class PaymentController {
    ...
    public String paymentPage(@PathVariable Long bookingId, Model model) {
        // Directly fetch the specific booking by ID using the repository or a dedicated service method // If you don't have getBookingById in service, you can use the repository directly
        Booking booking = bookingService.getBookingById(bookingId); // Ensure this method exists
    ...
    public String processPayment(@RequestParam("bookingId") Long bookingId, @ModelAttribute Payment payment) {
        // Directly fetch the specific booking by ID to ensure consistency and performance
    ...
    public String myPayments(@AuthenticationPrincipal UserDetails userDetails,
        ...
        Pageable pageable = PageRequest.of(page, 10, Sort.by("paymentId").descending());
        
        Page<Payment> paymentPage = paymentService.getUserPayments(user, status, pageable);
    ...
    public String adminPayments(@RequestParam(defaultValue = "0") int page,
                                ...
                                Model model) {
        Pageable pageable = PageRequest.of(page, 10, Sort.by("paymentId").descending());
        Page<Payment> paymentPage = paymentService.getAllPayments(status, pageable);
